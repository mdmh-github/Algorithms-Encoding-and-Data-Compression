...
///////////////////////////////////////////headers
void write_forward (int length, node * current_char, node * last);
node * get_next_node (FILE* inputFile, node * current_node);
void encode (char * input_file_name, char * output_file_name);
void backtrack (int offset,int length, node * current_char, node * last);
void write_forward (int length, node * current_char, node * last);
...
void encode (char * input_file_name, char * output_file_name)
{
	/// vars
	int end=0;
	node * begin_word =NULL;
	node *current_cursor=NULL;
	node * buffer =NULL;
	node  *buffer_cursor =NULL;
	FILE * inputFile;
	FILE * outputFile;
	int
	current_offset,
...
void decode (char * input_file_name, char * output_file_name){
	...
	int offset, size;
	int scape;
	node * last,* current_char,*first, * on_scrren;
...
int main(int argc,char * args []) {
	...
	default:
		printf ("error on decoding argument: use 0 for encode and 1 for decode.");
		return 0;
		break;
	}
