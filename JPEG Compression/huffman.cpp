...
void printProbabilities(node ** symbols, int length) {
	...
	for (current = 0; current < length; current++) {
		printf("\n%f\t\t\t%i\t%f\t%i\t", symbols[current]->symbol,
...
void printTree(FILE *f,node *symbol, int deep) {
	...
	fprintf(f,"%f\t\t\t%i\t%f\t", symbol->symbol, symbol->code,
			symbol->probability);
...
void printLeafs(node * symbol) {
	if (symbol->leaf != 1) {
		...
		printf("\n%f\t\t\t%i\t%f\t%i\t", symbol->symbol, symbol->code,
...
void save_codified(node ** codes, unsigned int codes_length,
		double * input_symbols, unsigned int input_symbols_length, FILE * file) {
	...
	for (unsigned int i = 0; i < input_symbols_length; i++) {
		...
		for (unsigned int j = 0; j < codes_length; j++) {
			//
			if ( fabs(input_symbols[i] - codes[j]->symbol)<0.0001) {
